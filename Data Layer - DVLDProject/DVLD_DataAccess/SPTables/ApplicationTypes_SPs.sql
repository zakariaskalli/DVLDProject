-- Stored Procedures for Table: ApplicationTypes

Use [DVLD];
Go


CREATE OR ALTER PROCEDURE SP_Get_ApplicationTypes_ByID
(
...
CREATE OR ALTER PROCEDURE SP_Get_All_ApplicationTypes
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_ApplicationTypes
(
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@ApplicationTypeTitle)) IS NULL OR LTRIM(RTRIM(@ApplicationFees)) IS NULL
        BEGIN
...
CREATE OR ALTER PROCEDURE SP_Update_ApplicationTypes_ByID
(
    @ApplicationTypeID int,
    @ApplicationTypeTitle nvarchar(150),
    @ApplicationFees smallmoney
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@ApplicationTypeTitle)) IS NULL OR LTRIM(RTRIM(@ApplicationTypeTitle)) = '' OR LTRIM(RTRIM(@ApplicationFees)) IS NULL OR LTRIM(RTRIM(@ApplicationFees)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_ApplicationTypes_ByID
(
    @ApplicationTypeID int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM ApplicationTypes WHERE ApplicationTypeID = @ApplicationTypeID)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Search_ApplicationTypes_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        WHEN 'ApplicationTypeTitle' THEN 'ApplicationTypeTitle'
        WHEN 'ApplicationFees' THEN 'ApplicationFees'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('ApplicationTypes') + 
