-- Stored Procedures for Table: TestTypes

Use [DVLD];
Go


CREATE OR ALTER PROCEDURE SP_Get_TestTypes_ByID
(
    @TestTypeID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_TestTypes
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_TestTypes
(
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@TestTypeTitle)) IS NULL OR LTRIM(RTRIM(@TestTypeDescription)) IS NULL OR LTRIM(RTRIM(@TestTypeFees)) IS NULL
        BEGIN
...
CREATE OR ALTER PROCEDURE SP_Update_TestTypes_ByID
(
    @TestTypeID int,
    @TestTypeTitle nvarchar(100),
    @TestTypeDescription nvarchar(500),
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@TestTypeTitle)) IS NULL OR LTRIM(RTRIM(@TestTypeTitle)) = '' OR LTRIM(RTRIM(@TestTypeDescription)) IS NULL OR LTRIM(RTRIM(@TestTypeDescription)) = '' OR LTRIM(RTRIM(@TestTypeFees)) IS NULL OR LTRIM(RTRIM(@TestTypeFees)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_TestTypes_ByID
(
    @TestTypeID int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM TestTypes WHERE TestTypeID = @TestTypeID)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM TestTypes WHERE TestTypeID = @TestTypeID;
...
CREATE OR ALTER PROCEDURE SP_Search_TestTypes_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        WHEN 'TestTypeTitle' THEN 'TestTypeTitle'
        WHEN 'TestTypeDescription' THEN 'TestTypeDescription'
        WHEN 'TestTypeFees' THEN 'TestTypeFees'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('TestTypes') + 
