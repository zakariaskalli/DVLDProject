-- Stored Procedures for Table: Countries

Use [DVLD];
Go


CREATE OR ALTER PROCEDURE SP_Get_Countries_ByID
(
    @CountryID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_Countries
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_Countries
(
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@CountryName)) IS NULL
        BEGIN
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@CountryName)) IS NULL OR LTRIM(RTRIM(@CountryName)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_Countries_ByID
(
    @CountryID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Search_Countries_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('Countries') + 
